...
class RafDataset(data.Dataset):
    def __init__(self, args, phase, basic_aug=True, transform=None):
        ...
        df = pd.read_csv(self.raf_path + 'EmoLabel/' + args.label_path, sep=' ', header=None)
        ...
        if phase == 'train':
            ...
            df = pd.read_csv(self.raf_path + 'EmoLabel/list_patition_label.txt', sep=' ', header=None)
        ...
        for f in images_names:
            f = f.split(".")[0]
            f += '_aligned.jpg'
            file_name = self.raf_path + 'Image/aligned/' + f
